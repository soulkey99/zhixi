...
function selectOption(obj,choice_num){
    ...
    if("rgb(240, 242, 246)"==$(obj).css("backgroundColor")){ //选中当前选项
        ...
        $(obj).css({"backgroundColor":"#f0f2f6","color":"#828b97"});
        $(obj).find("img").attr("src","images/selectOut.png");
        $(".mainBtn").css("backgroundColor","#5c6bc0");
        $("#btnText").text("查看题干");
        $(".mainBtn").unbind("click");
        $(".mainBtn").bind("click",mainQuestion);
    ...
    $.ajax(
        {
            type : "POST",
            async : true,
            url : "/rest/s/homework/"+ vm.swork_id() +"/question/"+ vm.nowQuestion().q_id +"/check",
            dataType : 'json',
            data: {
...
var viewModel = function() {
    ...
    this.swork_id = ko.observable(sessionStorage.getItem("swork_id"));
