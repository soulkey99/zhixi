...
let ossPut1 = function (opt, callback) { //thunkify oss put
    return function (callback) {
        oss.putObject(opt, callback);
    }
};
let qnPut = function (opt, callback) {  //opt = {key: '', localFile: ''}, thunkify qn put
...
module.exports = function *() {
    ...
    let filePath = this.request.body.fields.path || path.join('zhixi', dateStr, rack() + path.extname(fileName).toLowerCase()).replace(/\\/g, '/');
