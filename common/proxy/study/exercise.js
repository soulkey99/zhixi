...
exports.genExercise = function (param, callback) {
    ...
    return new Promise((resolve, reject)=> {
        model.StudyQuestion.findById(param.q_id, (err, question)=> {
            ...
            if (!question) {
                let err = new Error('q_id问题不存在！');
                callback && callback(err);
                return reject(err);
            }
            if (question.type != 'root') {
                let err = new Error('q_id对应问题类型不正确！');
                callback && callback(err);
                return reject(err);
            }
    ...
    return new Promise((resolve, reject)=> {
        ...
        ep.all('e', 'q', (e, q)=> {
            ...
            if (e.userID != param.userID) {
                let err = new Error('不是自己的练习！');
                callback && callback(err);
                return reject(err);
            }
            if (q.type == 'root') {
                let err = new Error('q_id类型不正确！');
                callback && callback(err);
                return reject(err);
            }
        ...
        Exercise.findById(id, (err, doc)=> {
            ...
            if (doc.status != 'finished') {
                doc.save((err2, doc2)=> {
                    ...
                    callback && callback(null, {e_id: doc2.e_id, point: doc2.point, percent: 20});
                    resolve({e_id: doc2.e_id, point: doc2.point, percent: 20});
                });
            }
        });
    });
        ...
        Exercise.findById(id, (err, e)=> {
            ...
            model.StudyQuestion.find({_id: {$in: q_ids}}, (err2, doc2)=> {
                ...
                if (lastStep.question.type != 'root') {
                    ...
                    callback && callback(null, exercise);
                    resolve(exercise);
                }
            });
        });
