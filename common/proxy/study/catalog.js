    ...
    return new Promise((resolve, reject)=> {
        getChapterOrNew(param.cha_id, (err, chapter)=> {
            ...
            if (param.action == 'remove') {
                Chapter.findByIdAndRemove(chapter._id, (err, doc)=> {
                    if (err) {
                        callback && callback(err);
                        return reject(err);
                    }
    ...
    return new Promise((resolve, reject)=> {
        if (!param.sec_id) {    //没传sec_id，那么就是新增小节的操作，需要将相应的小节挂载到对应的章下方
            ...
            Chapter.findById(param.cha_id, (err, chapter)=> {
                ...
                if (!chapter) {
                    let err = new Error('cha_id对应的内容不存在！');
                    callback && callback(err);
                    return reject(err);
                }
            ...
            Section.findById(param.sec_id, (err, section)=> {
                ...
                if (!section) {
                    let err = new Error('sec_id对应内容不存在！');
                    callback && callback(err);
                    return reject(err);
                }
                if (param.action == 'remove') {
                    Chapter.find({sections: section._id}, (err, doc)=> {
                        ...
                        Section.findByIdAndRemove(section._id, (err, doc)=> {
                            if (err) {
                                callback && callback(err);
                                return reject(err);
                            }
...
exports.editSectionQuestion = function (param, callback) {
    return new Promise((resolve, reject)=> {
        if (!param.q_id) {
            let err = new Error('q_id不能为空！');
            callback && callback(err);
            return reject(err);
        }
        if (!param.sec_id) {
            let err = new Error('sec_id不能为空！');
            callback && callback(err);
            return reject(err);
        }
        Section.findById(param.sec_id, (err, section)=> {
            ...
            if (!section) {
                let err = new Error('sec_id对应的内容不存在！');
                callback && callback(err);
                return reject(err);
            }
            if (section.type != 'section') {
                let err = new Error('sec_id对应的类型错误！');
                callback && callback(err);
                return reject(err);
            }
            if (param.action == 'remove') {
...
exports.getSectionQuestion = function (param, callback) {
    ...
    return new Promise((resolve, reject)=> {
        Section.findById(param.sec_id, (err, section)=> {
            ...
            if (section.type != 'section') {
                let err = new Error('sec_id对应内容类型不正确！');
                callback && callback(err);
                return reject(err);
            }
            let q_ids = [];
